...
void Server::handleStudentExamRequest(int sock, ExamManager exam) {
    ...
    int examNumber = atoi(buffer); // Convert received number to int
    ...
    // Extract exam name from the exams vector
    string selectedExamName;
    istringstream iss(exams[examNumber - 1]); // Get the correct exam entry
    string line;
    
    while (getline(iss, line)) {
        if (line.find("Exam Name:") != string::npos) {
            selectedExamName = line.substr(line.find(":") + 2); // Extract exam name
            break;
        }
    }
